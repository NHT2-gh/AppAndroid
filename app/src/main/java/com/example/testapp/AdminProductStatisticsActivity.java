...
//        Toast.makeText(AdminProductStatisticsActivity.this, "Thành công", Toast.LENGTH_SHORT).show();
    ...
    private void getDataStatistic(String token) {
        ApiService.apiservice.getStatisticProduct(token).enqueue(new Callback<ListEntityStatusResponse<ProductSaleRequest>>() {
            @Override
        ...
        ApiService.apiservice.getStatisticProductByDate(token, startDate, endDate).enqueue(new Callback<ListEntityStatusResponse<ProductSaleRequest>>() {
            ...
            public void onResponse(Call<ListEntityStatusResponse<ProductSaleRequest>> call, Response<ListEntityStatusResponse<ProductSaleRequest>> response) {
                if(response.isSuccessful()){
                    ...
                    ListEntityStatusResponse<ProductSaleRequest> dataResponse = response.body();
